...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const productosVendidos = JSON.parse(contenedor.getAttribute('data-productos-vendidos'));

    // Iterar sobre los productos vendidos para agregarlos al array de productos seleccionados
    productosVendidos.forEach(detalle => {
        const producto = {
            id: detalle.producto.id,
            nombre: detalle.producto.nombre,
            cantidad: detalle.cantidad,
            precio: detalle.producto.precio_de_venta,
            tipoVenta: detalle.tipo_venta,
            tipoPresentacion: detalle.id_presentacion
        };

        // Añadir cada producto vendido al array de productos seleccionados
        productosSeleccionados.push(producto);
    });

    // Actualizar carrito y renderizar productos ya vendidos
    actualizarCarrito();
    renderizarProductos(); // Asegúrate de que se llame aquí
});

// función que se ejecuta al hacer click en el botón de agregar producto
document.getElementById('agregarProducto').addEventListener('click', () => {
    let productoSelect = document.getElementById('productos');
    let tipoVentaSelect = document.getElementById('tipo_venta');
    let presentacion = productoSelect.options[productoSelect.selectedIndex].getAttribute('data-presentacion');
    ...
    //editar para que tome el precio unitario por ml/comprimido o el precio del producto completo
    let productoPrecio = 0;
    let cantidad = 0;

    if (tipoVentaSelect.value === 'completo') {
        cantidad = parseInt(document.getElementById('cantidad').value);
        productoPrecio = productoSelect.options[productoSelect.selectedIndex].getAttribute('data-precio');
    } else if (tipoVentaSelect.value === 'fraccionado') {
        if (presentacion == '1') {
            cantidad = parseInt(document.getElementById('cantidad_comprimidos').value);
            productoPrecio = productoSelect.options[productoSelect.selectedIndex].getAttribute('data-precioFraccionado');
...
function eliminarProducto(event, productoId) {
    ...
    // Convertimos ambos a string para asegurar la consistencia
    productoId = String(productoId);

    // Filtramos el array asegurándonos de eliminar el producto con el ID correspondiente
    productosSeleccionados = productosSeleccionados.filter(producto => String(producto.id) !== productoId);
...
function actualizarCarrito() {
    ...
    // Iterar sobre los productos seleccionados y calcular el subtotal
    productosSeleccionados.forEach((producto) => {
        ...
        subtotal += precioDecimal * producto.cantidad; // Multiplicar por cantidad
    });
...
function actualizarTotal() {
    ...
    let subtotal = parseFloat(subtotalSinPuntos) || 0;
